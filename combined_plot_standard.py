...
ddir="/gws/nopw/j04/cpdn_rapidwatch/joe_scratch/remapped/"
bfolder="/region/"
output={"Wind":("item3249_daily_mean","(m/s)"),"Precip":("item5216_daily_mean","(mm/day)"),"Tmax":("item3236_daily_maximum","(K)")}


def read_data(batch,diag,time_proc,resolution,subregion,umid_limits):
    files=glob.glob(ddir+"batch_"+str(batch)+'/'+output[diag][0]+"/remapped_"+output[diag][0]+"*.nc")
    mean_vals=[]
    #print files
	...
	#remap=' -remapbil,mygrid'
    else:
        remap=''

    if subregion != '':
        subsreg=' -sellonlatbox,'+subregion
    else:
        subreg=''
    ...
    for ifile in files:
		...
		#boxstr='-6,1.5,49.5,55'
		#cdo.sellonlatbox(boxstr, input='/home/users/ssparrow01/remap_test_v2.nc', returnCdf=True)
		#print 'check point'
		inputstr= ifile 
        ...
        # Compute the field mean value timeseries and return it as a numpy array
	     #   vals=cdo.selmon('7/9', input=' -fldmean '+inputstr+
		#	" "+ifile,returnCdf=True).variables[output[diag][0]][:]
		vals=cdo.selname(output[diag][0],options = '-f nc',input=inputstr,returnCdf=True).variables[output[diag][0]][:]
		

		#vals=Cdo(input=inputstr).variables[output[diag][0]][:]
		#vals=cdo.select(
		#print vals.shape
...
def plot_figure(diag,time_proc,res,subr,errb,restore):
    ...
    if restore==False:
        ...
        np.save('batch_646_data_' + output[diag][0] + '_full_field.npy',np.array(dataHist))
        np.save('batch_647_data_' + output[diag][0] + '_full_field.npy',np.array(data15))
        np.save('batch_648_data_' + output[diag][0] + '_full_field.npy',np.array(data2))
    else:
        dataHist=np.load('batch_646_data.npy')
        data15=np.load('batch_647_data.npy')
        data2=np.load('batch_648_data.npy')
    print(restore)
    dataHistAug=dataHist[:,30:59].flatten()
    data15Aug=data15[:,30:59].flatten()
    data2Aug=data2[:,30:59].flatten()
    dataHist=[]
    data15=[]
    data2=[]
    plot_distribution_data(dataHistAug,data15Aug,data2Aug,diag,fig)
    plot_return_time_data(dataHistAug,data15Aug,data2Aug,errb,diag,fig)
    plt.tight_layout()
    fig.savefig("combined_figure_"+str(diag)+"_full_field_Aug.png",dpi=28.75*2)
...
def plot_data(data,cols,plabel,ax,errb,diag):
    ...
    boot_all = calc_bootstrap_ensemble(data,direction="descending",bsn=1000)
    boot_all_x = calc_bootstrap_ensemble(x_data_all,direction="descending",bsn=1000)
...
def calc_bootstrap_ensemble(em, direction="ascending", bsn=1e3):
